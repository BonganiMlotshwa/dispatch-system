...
/**
 * Exit Scan Modal
 * Streamlined workflow for exit scanning with driver information
 */
const ExitScanModal = ({ show, onHide, onSuccess }) => {
  const [step, setStep] = useState(1); // 1=driver info, 2=scanning
  ...
  const [driverInfo, setDriverInfo] = useState({
    ...
    shipment_week: `Wk${Math.ceil((new Date().getDate()) / 7)}` // Current week
  });
  ...
  const handleDriverInfoSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/quick_truck.php`, {
      ...
      if (response.data.success) {
        // Store truck info in localStorage for the scanner to use
        localStorage.setItem('active_truck', JSON.stringify(response.data.truck));
        
        // Call success callback with truck info
        if (onSuccess) {
          onSuccess({
        ...
        // Close modal and let user continue with normal scanning
        handleClose();
      }
    } catch (err) {
  ...
  const handleComplete = () => {
    if (onSuccess) {
      onSuccess({
        ...
        totalUnits: scannedCartons.reduce((sum, c) => sum + parseInt(c.units || 0), 0)
      });
    }
    handleClose();
  };
  ...
  const totalUnits = scannedCartons.reduce((sum, c) => sum + parseInt(c.units || 0), 0);
          ...
          <Form onSubmit={handleDriverInfoSubmit}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Text className="text-muted">
                Enter your employee pin (e.g., 12901)
              </Form.Text>
            </Form.Group>
